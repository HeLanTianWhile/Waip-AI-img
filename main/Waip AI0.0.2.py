    ...
    for h in range(min_height):
        for g in range(min_width):
            ...
            for f in range(3):
                ...
                TF = False
                for d in ys:
                    if ys.count(d) > round(len(ys) / 2) + random.randint(-2, 2):
                        TF = True
                        if ys2 == "red":
                            r_ys = d
                        elif ys2 == "green":
                        ...
                        break
                if not TF:
                    if random.randint(0,10) < 7 and h > 0:
                        r_ys = img_xy[h - 1][g][0]
                        g_ys = img_xy[h - 1][g][1]
                        b_ys = img_xy[h - 1][g][2]
                    else:
                        # 随机选择一个值
                        if ys2 == "red":
                            r_ys = random.choice(r_values)
                        elif ys2 == "green":
                            g_ys = random.choice(g_values)
                        elif ys2 == "blue":
                            b_ys = random.choice(b_values)
