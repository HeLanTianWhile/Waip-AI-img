...
def Waip_imgea(user_input):
    ...
    for k in range(min_height):
        ...
        for g in range(min_width):
            ...
            for img_data in xy:
                ...
                ggg.append(pixel[1])
                bbb.append(pixel[2])
            rrr2 = sum(rrr) // len(rrr)
            ggg2 = sum(ggg) // len(ggg)
            bbb2 = sum(bbb) // len(bbb)
            img_xy[k].append([rrr2, ggg2, bbb2])  # 使用列表而不是元组，因为列表可以修改
    ...
    for h in range(min_height):
        for g in range(min_width):
            ...
            r_values.append(img_xy[h][g][0])
            g_values.append(img_xy[h][g][1])
            b_values.append(img_xy[h][g][2]) 
